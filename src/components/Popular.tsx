...
const Popular = () => {
  ...
  return (
    <section id="popular" className="py-12 sm:py-16 md:py-24 bg-gradient-warm">
      <div className="container px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-6">
          {popularItems.map((item, index) => (
            <div
              key={item.name}
              className="group relative bg-card rounded-xl sm:rounded-2xl overflow-hidden shadow-card hover:shadow-lg transition-all animate-fade-up"
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-2.5 sm:p-4">
                <h3 className="text-sm sm:text-lg font-bold text-card">{item.name}</h3>
                <p className="text-card/80 text-xs sm:text-sm hidden sm:block">{item.description}</p>
              </div>
            </div>
          ))}
        </div>
